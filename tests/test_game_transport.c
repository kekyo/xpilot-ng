...
static int check_parse(void)
{
    ...
    TEST_CHECK(Game_transport_parse("TcP", &transport));
    TEST_CHECK(transport == GAME_TRANSPORT_TCP);
    TEST_CHECK(Game_transport_parse("websocket", &transport));
    TEST_CHECK(transport == GAME_TRANSPORT_WEBSOCKET);
    TEST_CHECK(Game_transport_parse("WS", &transport));
    TEST_CHECK(transport == GAME_TRANSPORT_WEBSOCKET);
...
static int check_names(void)
{
    ...
    TEST_CHECK(strcmp(Game_transport_name(GAME_TRANSPORT_WEBSOCKET),
                      "websocket") == 0);
...
static int check_protocol_versions(void)
{
    ...
    TEST_CHECK(GAME_PROTOCOL_TCP_SESSION_POLYGON_VERSION == 0x4F1A);
    TEST_CHECK(GAME_PROTOCOL_TCP_SESSION_LEGACY_VERSION == 0x4506);
    TEST_CHECK(GAME_PROTOCOL_WEBSOCKET_SESSION_POLYGON_VERSION == 0x4F1B);
    TEST_CHECK(GAME_PROTOCOL_WEBSOCKET_SESSION_LEGACY_VERSION == 0x4507);
    ...
    TEST_CHECK(Game_transport_protocol_version(
                   GAME_TRANSPORT_WEBSOCKET, true)
               == GAME_PROTOCOL_WEBSOCKET_SESSION_POLYGON_VERSION);
    TEST_CHECK(Game_transport_protocol_version(
                   GAME_TRANSPORT_WEBSOCKET, false)
    ...
    TEST_CHECK(Game_transport_pair_is_supported(
    ...
    TEST_CHECK(Game_transport_pair_is_supported(
                   ...
                   GAME_TRANSPORT_WEBSOCKET));
    TEST_CHECK(!Game_transport_pair_is_supported(
                    GAME_TRANSPORT_WEBSOCKET, GAME_TRANSPORT_TCP));
    TEST_CHECK(!Game_transport_pair_is_supported(
                    GAME_TRANSPORT_UDP, GAME_TRANSPORT_WEBSOCKET));
    return 0;
}
...
static int check_meta_versions(void)
{
    ...
    TEST_CHECK(Game_transport_format_meta_version(
                   version, sizeof(version), "4.7.3infinity",
                   GAME_TRANSPORT_TCP, GAME_TRANSPORT_UDP));
    TEST_CHECK(strcmp(version, "4.7.3infinity+ct=tcp+gt=udp") == 0);
    ...
    TEST_CHECK(base_length == strlen("4.7.3infinity"));

    TEST_CHECK(Game_transport_format_meta_version(
                   version, sizeof(version), "4.7.3infinity",
                   GAME_TRANSPORT_UDP, GAME_TRANSPORT_TCP));
    TEST_CHECK(strcmp(version, "4.7.3infinity+ct=udp+gt=tcp") == 0);
    TEST_CHECK(Game_transport_parse_meta_version(
                   version, &contact, &gameplay, &base_length));
    ...
    TEST_CHECK(Game_transport_format_meta_version(
                   ...
                   GAME_TRANSPORT_WEBSOCKET,
                   GAME_TRANSPORT_WEBSOCKET));
    TEST_CHECK(strcmp(version,
                      "4.7.3infinity+ct=websocket+gt=websocket") == 0);
    TEST_CHECK(Game_transport_parse_meta_version(
                   version, &contact, &gameplay, &base_length));
    TEST_CHECK(contact == GAME_TRANSPORT_WEBSOCKET);
    TEST_CHECK(gameplay == GAME_TRANSPORT_WEBSOCKET);

    TEST_CHECK(!Game_transport_format_meta_version(
                    version, 8, "4.7.3infinity",
    ...
    TEST_CHECK(!Game_transport_format_meta_version(
                    ...
                    GAME_TRANSPORT_WEBSOCKET, GAME_TRANSPORT_TCP));
    TEST_CHECK(!Game_transport_parse_meta_version(
                    "4.7.3infinity", &contact, &gameplay, &base_length));
    TEST_CHECK(!Game_transport_parse_meta_version(
                    "4.7.3infinity+ct=tcp+gt=quic",
                    &contact, &gameplay, &base_length));
    TEST_CHECK(!Game_transport_parse_meta_version(
                    "4.7.3infinity+ct=websocket+gt=tcp",
                    &contact, &gameplay, &base_length));
    TEST_CHECK(!Game_transport_parse_meta_version(
                    NULL, &contact, &gameplay, &base_length));
    TEST_CHECK(!Game_transport_parse_meta_version(
                    "4.7.3infinity+ct=tcp+gt=tcp", NULL,
