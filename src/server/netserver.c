 ...
 * ordered reliable delivery, so this layer is redundant at the transport
 * level, but keeping it avoids changing gameplay packet formats and setup
 * transitions as part of the transport conversion.
 */
 ...
 * it is thrown out of the game so we send it a quit packet when its TCP
 * stream has already been accepted.  Since 3.0.6 the client receives a
 * short message explaining why the connection was terminated.
 */
void Destroy_connection(connection_t *connp, const char *reason)
{
	...
	if (Packet_printf(&connp->w, "%c%s", PKT_QUIT, (char *)reason) > 0)
