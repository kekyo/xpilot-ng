...
static void release_input_state(void)
{
	...
	if (target != NULL && target->button != NULL)
	    target->button((Uint8)button, SDL_RELEASED, x, y,
			   target->buttondata);
...
static void process_mouse_button(Uint8 button, Uint8 state, int x, int y)
{
    GLWidget *target;
 ...
 * events.  SDL2 uses the same values for physical X1 and X2 buttons. */
static bool is_sdl2_side_button(Uint8 button)
{
...
int Process_event(SDL_Event *evt)
{
	...
	if (is_sdl2_side_button(button))
	    break;
	process_mouse_button(button, evt->button.state,
	...
	if (evt->wheel.direction == SDL_MOUSEWHEEL_FLIPPED)
	    wheel_y = -wheel_y;
	SDL_GetMouseState(&x, &y);
	button = wheel_y > 0 ? 4 : 5;
	while (wheel_y != 0) {
	    process_mouse_button(button, SDL_PRESSED, x, y);
	    process_mouse_button(button, SDL_RELEASED, x, y);
	    wheel_y += wheel_y > 0 ? -1 : 1;
	}
	break;
