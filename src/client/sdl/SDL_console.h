...
extern "C" {
	...
	extern SDL_DECLSPEC SDL_Event* SDLCALL CON_Events(SDL_Event *event);
	/*! Makes the console visible */
	extern SDL_DECLSPEC void SDLCALL CON_Show(ConsoleInformation *console);
	/*! Hides the console */
	extern SDL_DECLSPEC void SDLCALL CON_Hide(ConsoleInformation *console);
	/*! Returns 1 if the console is opening or open, 0 else */
	extern SDL_DECLSPEC int SDLCALL CON_isVisible(ConsoleInformation *console);
	...
	extern SDL_DECLSPEC void SDLCALL CON_UpdateOffset(ConsoleInformation* console);
	/*! Draws the console to the screen if it is visible (NOT if it isVisible()). It get's drawn if it is REALLY visible ;-) */
	extern SDL_DECLSPEC void SDLCALL CON_DrawConsole(ConsoleInformation *console);
	...
	extern SDL_DECLSPEC ConsoleInformation* SDLCALL CON_Init(const char *FontName, SDL_Surface *DisplayScreen, int lines, SDL_Rect rect);
	/*! Frees the console's bitmap font and calls CON_Free */
	extern SDL_DECLSPEC void SDLCALL CON_Destroy(ConsoleInformation *console);
	/*! Frees all the memory loaded by the console */
	extern SDL_DECLSPEC void SDLCALL CON_Free(ConsoleInformation *console);
	/*! Function to send text to the console. Works exactly like printf and supports the same format */
	extern SDL_DECLSPEC void SDLCALL CON_Out(ConsoleInformation *console, const char *str, ...);
	/*! Sets the alpha level of the console to the specified value (0 - transparent,
		255 - opaque). Use this function also for OpenGL. */
	extern SDL_DECLSPEC void SDLCALL CON_Alpha(ConsoleInformation *console, unsigned char alpha);
	...
	extern SDL_DECLSPEC void SDLCALL CON_AlphaGL(SDL_Surface *s, int alpha);
	...
	extern SDL_DECLSPEC int SDLCALL CON_Background(ConsoleInformation *console, const char *image, int x, int y);
	/*! Changes current position of the console to the new given coordinates */
	extern SDL_DECLSPEC void SDLCALL CON_Position(ConsoleInformation *console, int x, int y);
	/*! Changes the size of the console. The previous surfaces remain active if resizing fails. */
	extern SDL_DECLSPEC int SDLCALL CON_Resize(ConsoleInformation *console, SDL_Rect rect);
	...
	extern SDL_DECLSPEC int SDLCALL CON_Transfer(ConsoleInformation* console, SDL_Surface* new_outputscreen, SDL_Rect rect);
	...
	extern SDL_DECLSPEC void SDLCALL CON_Topmost(ConsoleInformation *console);
	/*! Modify the prompt of the console. If you want a backslash you will have to escape it. */
	extern SDL_DECLSPEC void SDLCALL CON_SetPrompt(ConsoleInformation *console, const char* newprompt);
	/*! Set the key, that invokes a CON_Hide() after press. default is ESCAPE and you can always hide using
		ESCAPE and the HideKey (2 keys for hiding). compared against event->key.key !! */
	extern SDL_DECLSPEC void SDLCALL CON_SetHideKey(ConsoleInformation *console, int key);
	/*! Internal: executes the command typed in at the console (called if you press 'Return')*/
	extern SDL_DECLSPEC void SDLCALL CON_Execute(ConsoleInformation *console, char* command);
	...
	extern SDL_DECLSPEC void SDLCALL CON_SetExecuteFunction(ConsoleInformation *console, void(*CmdFunction)(ConsoleInformation *console2, char* command));
	...
	extern SDL_DECLSPEC void SDLCALL CON_SetTabCompletion(ConsoleInformation *console, char*(*TabFunction)(char* command));
	...
	extern SDL_DECLSPEC void SDLCALL CON_TabCompletion(ConsoleInformation *console);
	/*! Internal: makes a newline (same as printf("\n") or CON_Out(console, "\n") ) */
	extern SDL_DECLSPEC void SDLCALL CON_NewLineConsole(ConsoleInformation *console);
	/*! Internal: shift command history (the one you can switch with the up/down keys) */
	extern SDL_DECLSPEC void SDLCALL CON_NewLineCommand(ConsoleInformation *console);
	...
	extern SDL_DECLSPEC void SDLCALL CON_UpdateConsole(ConsoleInformation *console);


	/*! Internal: Default Execute callback */
	extern SDL_DECLSPEC void SDLCALL Default_CmdFunction(ConsoleInformation *console, char* command);
	/*! Internal: Default TabCompletion callback */
	extern SDL_DECLSPEC char* SDLCALL Default_TabFunction(char* command);
	...
	extern SDL_DECLSPEC void SDLCALL DrawCommandLine(void);

	/*! Internal: Gets called if you press the LEFT key (move cursor left) */
	extern SDL_DECLSPEC void SDLCALL Cursor_Left(ConsoleInformation *console);
	/*! Internal: Gets called if you press the RIGHT key (move cursor right) */
	extern SDL_DECLSPEC void SDLCALL Cursor_Right(ConsoleInformation *console);
	/*! Internal: Gets called if you press the HOME key (move cursor to the beginning
	of the line */
	extern SDL_DECLSPEC void SDLCALL Cursor_Home(ConsoleInformation *console);
	/*! Internal: Gets called if you press the END key (move cursor to the end of the line*/
	extern SDL_DECLSPEC void SDLCALL Cursor_End(ConsoleInformation *console);
	/*! Internal: Called if you press DELETE (deletes character under the cursor) */
	extern SDL_DECLSPEC void SDLCALL Cursor_Del(ConsoleInformation *console);
	/*! Internal: Called if you press BACKSPACE (deletes character left of cursor) */
	extern SDL_DECLSPEC void SDLCALL Cursor_BSpace(ConsoleInformation *console);
	/*! Internal: append one printable ASCII character to the command. */
	extern SDL_DECLSPEC void SDLCALL Cursor_Add(ConsoleInformation *console, char character);
    	
	/* add string to commandline */
	extern SDL_DECLSPEC void SDLCALL Add_String_to_Console(const char *text);
    	
	/*! Internal: Called if you press Ctrl-C (deletes the commandline) */
	extern SDL_DECLSPEC void SDLCALL Clear_Command(ConsoleInformation *console);
	/*! Internal: Called if the command line has changed (assemles console->Command from LCommand and RCommand */
	extern SDL_DECLSPEC void SDLCALL Assemble_Command(ConsoleInformation *console);
	/*! Internal: Called if you press Ctrl-L (deletes the History) */
	extern SDL_DECLSPEC void SDLCALL Clear_History(ConsoleInformation *console);

	/*! Internal: Called if you press UP key (switches through recent typed in commands */
	extern SDL_DECLSPEC void SDLCALL Command_Up(ConsoleInformation *console);
	/*! Internal: Called if you press DOWN key (switches through recent typed in commands */
	extern SDL_DECLSPEC void SDLCALL Command_Down(ConsoleInformation *console);
